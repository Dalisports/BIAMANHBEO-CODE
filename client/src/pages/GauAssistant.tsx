    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto bg-card rounded-3xl border border-border p-4 mb-4 min-h-[400px]" style={{ scrollbarWidth: 'thin', scrollbarColor: '#fbbf24 transparent' }}>
        {isVoiceMode && voiceInput && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center mb-3">
            ...
            <p className="text-base text-amber-900">"{voiceInput}"</p>
          </div>
        )}
      ...
      <div className="flex gap-3">
        <input
          ref={inputRef}
          type="text"
          value={isVoiceMode ? voiceInput || input : input}
          onChange={(e) => !isVoiceMode && setInput(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && sendMessage()}
          placeholder={isVoiceMode ? "🎤 Đang nghe..." : "Nhắn cho Gấu... ví dụ: 'Order bàn 5: 2 gà rán'"}
        ...
        <button
          ...
          disabled={isLoading}
          className={cn(
            "px-4 py-3 rounded-2xl transition-all flex items-center justify-center",
            isVoiceMode
        ...
        <button
          onClick={() => sendMessage()}
          disabled={(!input.trim() && !voiceInput.trim()) || isLoading}
